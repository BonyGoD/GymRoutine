# CLAUDE.md — GymRoutine

> Mirror of `AGENTS.md` (opencode) with the same content, plus the hard rules at the bottom.
...
**Repository result convention**: `RoutineRepository` returns `Result<T>` via `runCatching { ... }.mapError()`. Use cases are thin `operator fun invoke` wrappers. Follow that shape for new repositories.
...
- Writes go through `toMap(): Map<String, Any>` mappers in `data/mapper/`; reads go through `@Serializable` DTOs + `toDomain()` in `domain/mapper/`.
...
`kotlinx-datetime 0.8.0` is available in `commonMain`. Use `kotlin.time.Clock.System.now()` (see `LogWorkoutUseCase`) — not `kotlinx.datetime.Clock`, which is deprecated in 0.8.0.
...
Match the surrounding style by hand: 4-space indent, trailing commas on multi-line argument lists, imports sorted alphabetically, no unused imports.
...
- **String resources live in three locales** — `composeResources/values/`, `values-es/`, `values-ca/`. Any string added or removed must be applied to all three.
